...
def main(args):
    requests.post(
        f"http://localhost:8868/complete?info=" + f"用户{args.user_name}的scTCHCN训练任务{args.task_name}正在处理")
    try:
        # 设置随机种子
        ...
        requests.post(f"http://localhost:8868/complete?info=" + f"用户{args.user_name}的scTCHCN训练任务{args.task_name}正在进行数据预处理")
        # 注意：算法2只使用rna_path，atac_path在此处传入但dataset内部会处理
        Dataset.split_dataset(
            rna_path=args.rna_path,
            atac_path=args.atac_path, # 接口保持一致，内部可忽略
            label_path=args.label_path,
            seed=args.seed,
            base_dir=args.output_path,
        )

        # 预训练阶段 (Teacher-Student Distillation) 保存到用户名/任务名/result/
        if not args.use_pretrained:
            requests.post(f"http://localhost:8868/complete?info=" + f"用户{args.user_name}的scTCHCN训练任务{args.task_name}正在进行预训练(蒸馏)")
            pretrain = PreTrain(
                base_dir=args.output_path,
        ...
        requests.post(f"http://localhost:8868/complete?info=" + f"用户{args.user_name}的scTCHCN训练任务{args.task_name}正在训练分类器")
        pretrain_model_path = args.pretrain_path
        if not args.use_pretrained:
            # 算法2预训练保存的是student model
            pretrain_model_path = f"{args.output_path}result/pretrain_student_best.pt"
        ...
        requests.post(f"http://localhost:8868/complete?info=" + f"用户{args.user_name}的scTCHCN训练任务{args.task_name}训练完成，模型已保存至 {args.output_path}")
        ...
        try:
            ...
            payload = {
                ...
                "pretrainModelPath": f'{args.output_path}result/pretrain_student_best.pt',
                "defaultParameters": args.parameters,
        ...
        requests.post(f"http://localhost:8868/complete?info=" + f"用户{args.user_name}的scTCHCN训练任务{args.task_name}出错：{e}")
        # 设置任务为错误
...
if __name__ == "__main__":
    ...
    # 解析命令行参数
    parser = argparse.ArgumentParser(description='Run scTCHCN model training.')
    ...
    parser.add_argument('--batch_size', type=int, default=12, help='Batch size (smaller for graph models).')
    parser.add_argument('--patience', type=int, default=8, help='Early stopping patience for classifier training.')
    parser.add_argument('--input_dim', type=int, default=10000, help='Input dimension (RNA).')
    # 其他参数保持兼容，但可能在scTCHCN中未被全部使用
    parser.add_argument('--num_layers', type=int, default=2, help='Number of transformer layers.')
    parser.add_argument('--nhead', type=int, default=4, help='Number of attention heads.')
    parser.add_argument('--lr', type=float, default=1e-3, help='Learning rate.')
    parser.add_argument('--weight_decay', type=float, default=5e-3, help='Weight decay.')
    parser.add_argument('--atac_path', type=str, help='Path to the ATAC data file (h5ad).')
